...
const deleteItems = (itemId: number) => { //<<<<<<<<<<<<<<<<<<<<<
  order = {
    ...order!,
...
const renderOrder = (order: Order) => {
  const content = `
    <div>
      ...
      ${order.items.map(item => `
        <p class="bg-red-500">${item.amount} x ${pizzas.find(pizza => pizza.id === item.id)!.name}
        <button class="delete-button" data-item-id="${item.id}">Delete</button></p>
      `)}
  ...
  document.querySelector(".delete-button")!.addEventListener('click', deleteListener) //<<<<<<<<<<<<<<<<<<<<<
}
